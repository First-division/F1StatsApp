...
export default function ConstructorsScreen() {
  ...
  const { standings, loading, error, refetch } = useConstructorStandings(season);

  if (loading && standings.length === 0) {
    return <LoadingSpinner message="Loading constructor standings..." />;
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => <ConstructorStandingRow standing={item} />}
        refreshControl={
          <RefreshControl refreshing={loading} onRefresh={refetch} tintColor={Colors.primary} />
        }
        contentContainerStyle={styles.list}
        ListHeaderComponent={
          <Text style={styles.listHeader}>{standings.length} Constructors</Text>
        }
